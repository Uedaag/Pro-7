...
export const VideosView: React.FC = () => {
  ...
  const [editingVideo, setEditingVideo] = useState<VideoItem | null>(null); // Se null, é criação
    ...
    <div className="max-w-7xl mx-auto pb-20 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4 border-b border-slate-200 dark:border-white/10 pb-6">
        ...
        {isAdmin && (
          <button 
            ...
            className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-bold shadow-lg shadow-blue-600/20 flex items-center gap-2 transition-transform hover:-translate-y-0.5"
          >
      ...
      <div className="mb-8 space-y-4">
        <div className="relative">
          ...
          <input 
            ...
            className="w-full pl-12 pr-4 py-3 rounded-xl bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-white/10 focus:ring-2 focus:ring-blue-500 outline-none text-slate-800 dark:text-white shadow-sm"
          />
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          <button 
            ...
            className={`px-4 py-2 rounded-full text-sm font-bold transition-all border ${selectedCategory === 'Todos' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white dark:bg-[#0f172a] text-slate-600 dark:text-slate-300 border-slate-200 dark:border-white/10 hover:border-blue-400'}`}
          >
            Todos
          </button>
          {CATEGORIES.map(cat => (
            <button 
              ...
              className={`px-4 py-2 rounded-full text-sm font-bold transition-all border ${selectedCategory === cat ? 'bg-blue-600 text-white border-blue-600' : 'bg-white dark:bg-[#0f172a] text-slate-600 dark:text-slate-300 border-slate-200 dark:border-white/10 hover:border-blue-400'}`}
            >
              {cat}
            </button>
          ))}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-[#0f172a] rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800 text-center">
          ...
          <h3 className="text-xl font-bold text-slate-700 dark:text-slate-200">Nenhum vídeo encontrado</h3>
          ...
          {filteredVideos.map((video) => (
            ...
            >
              ...
              <div className="relative aspect-video bg-black group-hover:opacity-90 transition-opacity">
                ...
                <div className="absolute top-3 left-3 px-2 py-1 bg-black/60 backdrop-blur-md rounded text-[10px] font-bold text-white uppercase tracking-wide border border-white/10">
                ...
                <div className="absolute inset-0 flex items-center justify-center bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity backdrop-blur-[2px]">
                  <div className="w-16 h-16 bg-blue-600 rounded-full flex items-center justify-center shadow-2xl transform scale-75 group-hover:scale-100 transition-transform">
              ...
              <div className="p-5 flex-1 flex flex-col justify-between">
                ...
                <button className="text-blue-600 dark:text-blue-400 text-xs font-bold uppercase tracking-wider hover:underline mt-2 flex items-center gap-1 self-start">
                  Assistir Agora
                </button>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-[#0f172a] w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-scale-in">
            <div className="p-6 border-b border-slate-100 dark:border-white/5 flex justify-between items-center bg-slate-50 dark:bg-[#020410]">
              <h3 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                {editingVideo ? <Edit3 size={20} className="text-blue-600"/> : <Plus size={20} className="text-blue-600"/>}
                {editingVideo ? 'Editar Vídeo' : 'Novo Vídeo'}
              </h3>
              <button onClick={() => setIsFormOpen(false)} className="text-slate-400 hover:text-blue-500">
                <X size={20}/>
              </button>
            </div>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-5">
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-slate-800 dark:text-white"
                  placeholder="Ex: Aula sobre Cores"
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Tema / Disciplina</label>
                <select
                  required
                  value={formCategory}
                  onChange={e => setFormCategory(e.target.value)}
                  className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-slate-800 dark:text-white cursor-pointer"
                >
                  {CATEGORIES.map(cat => (
              ...
              <div className="flex justify-end pt-2">
                <button 
                  ...
                  className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg flex items-center gap-2 disabled:opacity-50 transition-all"
                >
                  {isSubmitting ? <Loader2 className="animate-spin" size={18} /> : <Save size={18} />}
                  {isSubmitting ? 'Salvando...' : (editingVideo ? 'Atualizar' : 'Salvar Vídeo')}
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="w-full max-w-5xl animate-scale-in relative">
            ...
            <div className="relative aspect-video bg-black rounded-2xl overflow-hidden shadow-2xl border border-white/10">
              <iframe 
                width="100%" 
                height="100%" 
                src={`https://www.youtube.com/embed/${extractYouTubeId(selectedVideoToPlay.url)}?autoplay=1&modestbranding=1&rel=0&origin=${window.location.origin}`} 
                title={selectedVideoToPlay.title}
                frameBorder="0" 
            ...
            <div className="mt-4 flex items-center justify-between">
              <h2 className="text-2xl font-bold text-white">{selectedVideoToPlay.title}</h2>
              <span className="px-3 py-1 bg-blue-600 text-white text-xs font-bold rounded-full">
                {selectedVideoToPlay.category || 'Geral'}
              </span>
            </div>
          </div>
        </div>
